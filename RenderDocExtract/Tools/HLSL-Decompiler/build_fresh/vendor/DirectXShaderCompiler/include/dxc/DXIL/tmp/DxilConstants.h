...
enum class OpCode : unsigned {
  ...
  MatVecMul = 305, // Multiplies a MxK dimension matrix and a K sized input vector
  MatVecMulAdd = 306, // multiplies a MxK dimension matrix and a K sized input vector and adds an M-sized bias vector
  OuterProductAccumulate = 307, // Computes the outer product between column vectors and an MxN matrix is accumulated component-wise atomically (with device scope) in memory
  VectorAccumulate = 308, // Accumulates the components of a vector component-wise atomically (with device scope) to the corresponding elements of an array in memory
  ...
  HitObject_FromRayQuery = 263, // Creates a new HitObject representing a committed hit from a RayQuery
  HitObject_FromRayQueryWithAttrs = 264, // Creates a new HitObject representing a committed hit from a RayQuery and committed attributes
  ...
  HitObject_Invoke = 267, // Represents the invocation of the CH/MS shader represented by the HitObject
  ...
  HitObject_LoadLocalRootTableConstant = 288, // Returns the root table constant for this HitObject and offset
  HitObject_MakeMiss = 265, // Creates a new HitObject representing a miss
  HitObject_MakeNop = 266, // Creates an empty nop HitObject
  HitObject_ObjectRayDirection = 278, // Returns the ray direction in object space
  HitObject_ObjectRayOrigin = 277, // Returns the ray origin in object space
  HitObject_ObjectToWorld3x4 = 279, // Returns the object to world space transformation matrix in 3x4 form
  HitObject_PrimitiveIndex = 284, // Returns the primitive index committed on hit
  HitObject_RayFlags = 272, // Returns the ray flags set in the HitObject
  HitObject_RayTCurrent = 274, // Returns the current T value set in the HitObject
  HitObject_RayTMin = 273, // Returns the TMin value set in the HitObject
  HitObject_SetShaderTableIndex = 287, // Returns a HitObject with updated shader table index
  HitObject_ShaderTableIndex = 286, // Returns the shader table index set for this HitObject
  HitObject_TraceRay = 262, // Analogous to TraceRay but without invoking CH/MS and returns the intermediate state as a HitObject
  ...
  HitObject_WorldToObject3x4 = 280, // Returns the world to object space transformation matrix in 3x4 form
