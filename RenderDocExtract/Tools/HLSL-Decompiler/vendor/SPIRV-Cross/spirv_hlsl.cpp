...
void CompilerHLSL::emit_buffer_block(const SPIRVariable &var)
{
	...
	{
		bool tightly_packed_scalar_array = false;
		if (type.member_types.size() == 1)
		{
			auto &member_type = get<SPIRType>(type.member_types[0]);
			tightly_packed_scalar_array = member_type.array.size() == 1 && member_type.columns == 1 &&
			                              member_type.vecsize == 1 && member_type.width == 32 &&
			                              type_struct_member_offset(type, 0) == 0 &&
			                              type_struct_member_array_stride(type, 0) == 4;
		}

		if (tightly_packed_scalar_array)
		{
			auto &member_type = get<SPIRType>(type.member_types[0]);
			size_t register_count = (get_declared_struct_size(type) + 15) / 16;
			SPIRType register_type { OpTypeVector };
			register_type.basetype = member_type.basetype;
			register_type.width = member_type.width;
			register_type.vecsize = 4;
			statement("cbuffer ", to_name(var.self), to_resource_binding(var));
			begin_scope();
			statement(type_to_glsl(register_type), " ", to_name(type.self, false), "[", register_count,
			          "] : packoffset(c0);");
			end_scope_decl();
			statement("");
		}
	...
	else
	{
		if (type.array.empty())
		{
			...
			{
				bool can_flatten_tightly_packed_scalar_array = false;
				if (type.member_types.size() == 1)
				{
					auto &member_type = get<SPIRType>(type.member_types[0]);
					can_flatten_tightly_packed_scalar_array = member_type.array.size() == 1 &&
					                                          member_type.columns == 1 && member_type.vecsize == 1 &&
					                                          member_type.width == 32 &&
					                                          (member_type.basetype == SPIRType::Float ||
					                                           member_type.basetype == SPIRType::Int ||
					                                           member_type.basetype == SPIRType::UInt) &&
					                                          type_struct_member_offset(type, 0) == 0 &&
					                                          type_struct_member_array_stride(type, 0) == 4;
				}

				if (can_flatten_tightly_packed_scalar_array)
				{
					flattened_buffer_blocks.insert(var.self);
				...
				auto &failed_type = get<SPIRType>(type.member_types[failed_index]);
				uint32_t actual_offset = type_struct_member_offset(type, failed_index);
				uint32_t actual_stride = failed_type.array.empty() ? 0 :
				                         type_struct_member_array_stride(type, failed_index);
				uint32_t hlsl_stride = failed_type.array.empty() ? 0 :
				                       type_to_packed_array_stride(failed_type,
				                                                   ir.meta[type.self].members[failed_index].decoration_flags,
				...
				SPIRV_CROSS_THROW(join("cbuffer ID ", var.self, " (name: ", buffer_name, "), member index ",
				                       ...
				                       ") cannot be expressed with HLSL packing. Actual offset=", actual_offset,
				                       ", actual array stride=", actual_stride, ", required HLSL array stride=", hlsl_stride,
				                       "."));
			}
