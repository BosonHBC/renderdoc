...
{
	std::string dynamic_component;
	auto result = flattened_access_chain_offset(expression_type(base), indices, count, offset, 16, nullptr, nullptr,
	                                          nullptr, false, &dynamic_component);
	...
	else
	{
		...
		if (!dynamic_component.empty())
		{
			if (target_type.vecsize != 1 || index % 4 != 0)
				SPIRV_CROSS_THROW("Tightly packed dynamic array access must resolve to an aligned scalar.");
			expr += "[" + dynamic_component + "]";
		}
		else
    ...
    std::string *dynamic_component)
{
	...
	for (uint32_t i = 0; i < count; i++)
	{
		...
		else if (!type->array.empty())
		{
			...
			{
				// A tightly packed scalar array can be represented as a vector array. Split the scalar index
				// into a vector register index and a component index while preserving the byte address.
				if (array_stride % word_stride)
				{
					if (!dynamic_component || !dynamic_component->empty() || array_stride * 4 != word_stride ||
					    (offset % word_stride) != 0)
					{
						SPIRV_CROSS_THROW(join("Cannot flatten dynamically indexed array: stride=", array_stride,
						                       " bytes, register width=", word_stride, " bytes, base offset=", offset,
						                       ". Only an aligned 32-bit scalar array can use component indexing."));
					}

					auto index_expr = to_enclosed_expression(index, false);
					expr += "(" + index_expr + " / 4) + ";
					*dynamic_component = "(" + index_expr + " & 3)";
				}
				else
				{
